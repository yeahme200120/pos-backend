...
const mesasFiltradas = computed(() => {
    ...
    return mesas.value.filter((mesa) => {
        ...
        const coincideEstado =
            !filtroEstado.value ||
            String(mesa.estado || '').toLowerCase() === filtroEstado.value;

        const coincideActivo =
            filtroActivo.value === '' ||
            String(Boolean(mesa.activo)) === filtroActivo.value;
...
const totalPaginas = computed(() => {
    return Math.max(
        1,
        Math.ceil(mesasFiltradas.value.length / porPagina)
    );
});
...
const mesasPaginadas = computed(() => {
    ...
    return mesasFiltradas.value.slice(
        inicio,
        inicio + porPagina
    );
});
...
const mesasLibres = computed(() => {
    return mesas.value.filter(
        (mesa) =>
            String(mesa.estado || '').toLowerCase() === 'libre' &&
            Boolean(mesa.activo)
    ).length;
});
...
const mesasOcupadas = computed(() => {
    return mesas.value.filter(
        (mesa) =>
            String(mesa.estado || '').toLowerCase() === 'ocupada'
    ).length;
});
...
const mesasReservadas = computed(() => {
    return mesas.value.filter(
        (mesa) =>
            String(mesa.estado || '').toLowerCase() === 'reservada'
    ).length;
});

const mesasInactivas = computed(() => {
    return mesas.value.filter(
        (mesa) => !Boolean(mesa.activo)
    ).length;
});
...
function mensajeError(
    error,
    mensajeDefault = 'Ocurrió un error.'
) {
...
function normalizarMesa(response) {
    const data = response?.data;
...
async function cargarMesas() {
    ...
    try {
        ...
        mesas.value = normalizarLista(response);
        ...
        await Swal.fire({
            ...
            html: mensajeError(
                error,
                'No fue posible obtener las mesas de la empresa.'
            ),
        });
    } finally {
...
function abrirModalEditar(mesa) {
    ...
    form.nombre = mesa.nombre || '';

    form.capacidad =
        mesa.capacidad !== null &&
        mesa.capacidad !== undefined
            ? mesa.capacidad
            : '';

    form.estado = mesa.estado || 'libre';
    form.activo = Boolean(mesa.activo);
    form.notas = mesa.notas || '';
...
function validarFormulario() {
    ...
    if (nombre.length > 255) {
        return 'El nombre de la mesa no puede superar los 255 caracteres.';
    }

    if (form.capacidad !== '') {
        const capacidad = numero(form.capacidad);
    ...
    if (!form.estado) {
        return 'Selecciona el estado de la mesa.';
    }
...
async function guardarMesa() {
    ...
    try {
        ...
        if (
            ...
            await api.put(
                `/mesas/${mesaSeleccionada.value.id}`,
                payload
            );
        } else {
...
async function cambiarEstado(mesa, nuevoEstado) {
    ...
    const confirmacion = await Swal.fire({
        ...
        html: `
            ...
            <p class="mb-0">
                Nuevo estado:
                <strong>${nombres[nuevoEstado] || nuevoEstado}</strong>
            </p>
        `,
    ...
    try {
        ...
        await api.put(`/mesas/${mesa.id}`, {
            nombre: mesa.nombre,
            capacidad: mesa.capacidad ?? null,
            estado: nuevoEstado,
            activo: Boolean(mesa.activo),
            notas: mesa.notas || null,
        });

        await Swal.fire({
            icon: 'success',
            title: 'Estado actualizado',
            text: `La mesa ahora está ${
                nombres[nuevoEstado] || nuevoEstado
            }.`,
            timer: 1600,
            showConfirmButton: false,
        });
...
async function cambiarActivo(mesa) {
    if (!mesa?.id) {
        return;
    }

    const nuevoEstado = !Boolean(mesa.activo);
    ...
    const confirmacion = await Swal.fire({
        ...
        title: nuevoEstado
            ...
            : 'Desactivar mesa',
        text: nuevoEstado
            ? `¿Deseas activar la mesa "${mesa.nombre}"?`
    ...
    try {
        ...
        await Swal.fire({
            ...
            html: mensajeError(
                error,
                'No fue posible cambiar el estado activo de la mesa.'
            ),
        });
    } finally {
        cargando.value = false;
...
// ======================================================
// SEGURIDAD PARA MENSAJES HTML DE SWEETALERT
// ======================================================
    ...
    >
        ...
        >
            <div>
                ...
                <p class="text-secondary mb-0">
                    Administra las mesas disponibles para la operación
                    del negocio.
                </p>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                ...
                <div
                    v-else-if="!mesasPaginadas.length"
                    class="empty-state"
                >
                    <i class="bi bi-grid-3x3-gap"></i>
                    ...
                    <p class="text-secondary mb-3">
                        No existen mesas que coincidan con los filtros
                        actuales.
                    </p>
                ...
                >
                    ...
                    >
                        <div
                            class="mesa-card"
                            :class="{
                                'mesa-inactiva': !mesa.activo
                            }"
                        >
                            <div
                                class="mesa-card-header"
                                :class="claseEstado(mesa.estado)"
                            >
                                ...
                                <div class="dropdown">
                                    ...
                                    >
                                        ...
                                        <li>
                                            ...
                                            >
                                                <i
                                                    class="bi bi-person-workspace me-2 text-warning"
                                                ></i>
                                                Ocupada
                                            </button>
                                        </li>
                                        ...
                                        <li>
                                            ...
                                            >
                                                <i
                                                    class="bi bi-slash-circle me-2 text-danger"
                                                ></i>
                                                Fuera de servicio
                                            </button>
                                        </li>
                        ...
                        <small class="text-secondary">
                            {{
                                editando
                                    ...
                                    : 'Registra una nueva mesa para la empresa.'
                            }}
                        </small>
                    </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <small
                                    class="text-secondary"
                                >
                                    Las mesas inactivas no deben
                                    utilizarse para nuevas ventas.
                                </small>
                            </div>
